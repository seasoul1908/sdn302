...
const { validateArticle, validateDate, validateTextLength } = require('../middlewares/articleValidator');
...
articleRouter.route('/')
    ...
    .post(validateArticle, validateDate, validateTextLength, async (req, res, next) => {
        try {
            ...
            res.status(201).json({ message: 'Added successful', article: newArticle });
        } catch (err) {
            next(err);
        }
    });
...
articleRouter.route('/:id')
    ...
    .put(validateArticle, validateDate, validateTextLength, async (req, res, next) => {
        try {
            const data = await readData();
            const id = parseInt(req.params.id);
